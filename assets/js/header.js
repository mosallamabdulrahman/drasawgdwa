function renderHeader() {
  ...
  const isFunding = page.includes("funding");
  const isFaq = page.includes("faq");
  const isTerms = page.includes("terms");
  const isPrivacy = page.includes("privacy");

  headerContainer.innerHTML = `
    <!-- 1. TOP BAR (Visible on Desktop >= lg only) -->
    <aside class="top-bar hidden md:block bg-[#17532a] text-white py-2 text-xs xl:text-sm border-b border-white/10 transition-all duration-300 relative z-50">
      <div class="max-w-7xl mx-auto px-4 md:px-8 flex justify-between items-center">
        <!-- 4 Pages Links -->
        <div class="flex items-center gap-4 text-white/90 text-xs xl:text-sm font-medium">
          <a href="faq.html" class="hover:text-orange transition-colors ${isFaq ? "text-orange font-bold" : ""}">الأسئلة الشائعة</a>
          <a href="terms.html" class="hover:text-orange transition-colors ${isTerms ? "text-orange font-bold" : ""}">شروط الاستخدام</a>
          <a href="privacy-policy.html" class="hover:text-orange transition-colors ${isPrivacy ? "text-orange font-bold" : ""}">سياسة الخصوصية</a>
          <a href="order-study.html" class="hover:text-orange transition-colors font-bold text-white">اطلب دراسة جدوى</a>
        </div>

        <!-- Social Media Icons (Desktop only) -->
        <div class="flex items-center gap-3.5 text-white/85 text-sm">
          <a href="https://wa.me/966534401649" target="_blank" aria-label="واتساب" class="hover:text-orange transition-colors p-0.5" title="واتساب">
          ...
          <a href="#" aria-label="لينكد إن" class="hover:text-orange transition-colors p-0.5" title="لينكد إن">
            <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24">
              <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
            </svg>
          </a>
          <a href="#" aria-label="إنستغرام" class="hover:text-orange transition-colors p-0.5" title="إنستغرام">
            <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24">
          ...
          <a href="#" aria-label="تويتر" class="hover:text-orange transition-colors p-0.5" title="تويتر">
            <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24">
      ...
      <header class="site-header bg-brand-900 border-b border-gray-200 h-20 sm:h-22 transition-all duration-300 flex items-center py-2.5 sm:py-3">
        <div class="max-w-7xl mx-auto px-4 md:px-8 w-full flex items-center justify-between">
          ...
          <nav class="hidden lg:flex items-center gap-6 xl:gap-7 font-bold text-base text-white" aria-label="القائمة الرئيسية">
            ...
            <a href="funding.html" class="nav-link-item ${isFunding ? "active font-bold text-orange" : ""} py-1 transition-colors">جهات التمويل</a>
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto py-4 px-4 space-y-1.5 text-right font-medium">
        ...
        <a
          href="funding.html"
          class="mobile-nav-link block px-4 py-3 rounded-xl ${isFunding ? "bg-brand-50 text-brand-900 font-bold" : "text-slate-700 hover:bg-slate-50 hover:text-brand-900"} transition-colors"
        >
          جهات التمويل
        </a>
        ...
        <!-- روابط إضافية (صفحات الهيدر العلوي في الموبايل) -->
        <div class="pt-3 mt-2 border-t border-slate-200/80 space-y-1">
          <a
            href="faq.html"
            class="mobile-nav-link flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-medium ${isFaq ? "bg-brand-50 text-brand-900 font-bold" : "text-slate-600 hover:bg-slate-50 hover:text-brand-900"} transition-colors"
          >
          ...
          <a
            href="terms.html"
            class="mobile-nav-link flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-medium ${isTerms ? "bg-brand-50 text-brand-900 font-bold" : "text-slate-600 hover:bg-slate-50 hover:text-brand-900"} transition-colors"
          >
          ...
          <a
            ...
            class="mobile-nav-link flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-medium ${isPrivacy ? "bg-brand-50 text-brand-900 font-bold" : "text-slate-600 hover:bg-slate-50 hover:text-brand-900"} transition-colors"
          >
