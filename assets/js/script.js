...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const searchCategoryPills = document.querySelectorAll(
  ...
  function getCategoryBadgeClass(cat) {
    switch (cat) {
      case "services":
        return "bg-brand-50 text-brand-900 border-brand-200";
      case "sectors":
  ...
  function renderSearchResults(query = "") {
    ...
    const resultElements = searchResultsList.querySelectorAll(
      ".search-result-item",
    );
      ...
      el.addEventListener("click", (e) => {
        ...
        if (targetId && targetId.startsWith("#")) {
          ...
          if (targetEl) {
            ...
            setTimeout(
              () => targetEl.classList.remove("ring-2", "ring-[#287B3F]"),
              2000,
            );
          }
        }
  ...
  searchInput?.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      if (
        ...
        selectedResultIndex < resultItems.length
      ) {
        e.preventDefault();
  ...
  if (
    ...
    document.querySelector(".sectorSwiper")
  ) {
    ...
    document
      .querySelectorAll(".sectorSwiper .swiper-slide")
      .forEach((slide) => {
  ...
  function activateFundingTab(index) {
    ...
    currentFundingIndex =
      (index + fundingTabBtns.length) % fundingTabBtns.length;
    ...
    const targetTabId =
      fundingTabBtns[currentFundingIndex]?.getAttribute("data-tab");
  ...
  if (statCounters.length > 0) {
    ...
    const statsContainer =
      statCounters[0].closest(".reveal") || statCounters[0].closest("#why-us");
