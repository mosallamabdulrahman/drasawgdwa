...
function generateSection() {
  ...
  steps.forEach((s) => {
    if (s.col === 'left') {
      // Visual Left means in RTL it's the 2nd child of the 2-col grid
      desktopRows += `
      ...
      // Visual Right means in RTL it's the 1st child of the 2-col grid
      desktopRows += `
  ...
  return `
      <!-- ==========================================================================
         7. كيف تتم الدراسة داخل دراسة وجدوى؟ (Process Timeline)
         ========================================================================== -->
      <section
        id="process"
        class="py-12 md:py-20 bg-[#f8faf8] relative overflow-hidden"
      >
        <!-- Background subtle decorative glow circles -->
        <div class="absolute top-1/4 right-0 w-96 h-96 bg-[#287b3f]/5 rounded-full blur-3xl pointer-events-none"></div>
        <div class="absolute bottom-10 left-0 w-96 h-96 bg-[#287b3f]/5 rounded-full blur-3xl pointer-events-none"></div>
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 relative z-10">
          ...
          <div class="text-center max-w-3xl mx-auto mb-12 md:mb-16 reveal">
            <span
              class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs sm:text-sm font-bold bg-[#eaf7ee] text-[#287b3f] border border-[#287b3f]/20 shadow-xs mb-3.5"
            >
            ...
            <h2
              class="text-2xl sm:text-4xl lg:text-5xl font-black text-brand-900 font-heading tracking-tight mb-4"
            >
              كيف تتم الدراسة داخل دراسة و جدوى؟
            </h2>
            <p
              class="text-sm sm:text-base md:text-lg text-slate-600 font-medium max-w-2xl mx-auto leading-relaxed"
            >
              خطوات متكاملة ومدروسة نتبعها في إعداد كل دراسة جدوى استثمارية لضمان الدقة العالية وقبول التمويل
            </p>
          </div>
...
const html = generateSection();
fs.writeFileSync('scratch/generated_process.html', html, 'utf8');
console.log('Successfully generated scratch/generated_process.html');
